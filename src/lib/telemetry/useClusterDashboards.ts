...
import { subscribeMachineNetworks } from '../machineNetworkStore';
...
export function useClusterDashboards(telemetry: TelemetryState, intervalMs: number = DEFAULT_INTERVAL_MS) {
  ...
  const [machineNetRevision, setMachineNetRevision] = useState(0);
  ...
  useEffect(() => subscribeMachineNetworks(() => setMachineNetRevision((value) => value + 1)), []);
  ...
  const bundle = useMemo(
    ...
    [dataSource, payload, simulationRevision, machineNetRevision, useLive],
  );
