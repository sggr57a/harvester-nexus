...
describe('storage dashboard data', () => {
  it('covers every backend named in the README plus the v2.0 Vitastor addition and AnyRAID', () => {
    const ids = buildStorageDashboard().backends.map((backend) => backend.id);
    for (const required of ['ceph', 'longhorn', 'nvme-of', 'rdma', 'zfs', 'anyraid', 'iscsi', 'nfs', 'smb', 'glusterfs', 'openebs', 'portworx', 'vitastor', 'local']) {
