...
export function EnvironmentIntelHudView({
  ...
  const thermalSeries = useRollingSeries(model.nodes[0]?.cpu ?? telemetry?.cpuPercent ?? 50, 48, telemetry?.tick);
  const powerSeries = useRollingSeries(telemetry?.watts ?? 1592, 48, telemetry?.tick);
  ...
  const diskSeries = useRollingSeries(
    telemetry ? telemetry.totalIops / 12000 : 50,
    48,
    telemetry?.tick,
  );
  const humSeries = useRollingSeries(telemetry?.ramPercent ?? 43, 48, telemetry?.tick);
    ...
    <section className="dash dash-hologram-hud dash-environment-holo" aria-label="Environment Intel hologram HUD">
      ...
      <header className="dash-header holo-header">
        ...
        <div className="dash-totals">
          ...
          <div><span>Power</span><strong>{Math.round(telemetry?.watts ?? 0)}W</strong></div>
          <div><span>Airflow</span><strong>{fmtMb(telemetry?.ingressMbps ?? 0)}</strong></div>
        </div>
      </header>
      ...
      <div className="holo-grid">
        ...
        <HudTile label="POWER" className="holo-gauge">
          <KpiTile label="PWR" value={`${Math.round(telemetry?.watts ?? 0)}`} unit="W" size="sm" series={powerSeries} />
        </HudTile>
        <HudTile label="AIRFLOW" className="holo-gauge">
          <KpiTile label="AIR" value={fmtMb(telemetry?.ingressMbps ?? 0)} size="sm" series={airSeries} status="warn" />
        </HudTile>
        ...
        <HudTile label="HUMIDITY" className="holo-gauge-sm">
          <KpiTile label="HUM" value={fmtPct(telemetry?.ramPercent ?? 43)} size="sm" series={humSeries} />
        </HudTile>
        ...
        <HudTile label="Linear strip" className="holo-linear-row">
          <div className="holo-linear-grid">
            ...
            <HudLinearBar label="HUM" value={telemetry?.ramPercent ?? 43} max={100} tone="accent-2" />
            <HudLinearBar label="IN" value={telemetry?.ingressMbps ?? 0} max={110_000} tone="warn" />
            <HudLinearBar label="OUT" value={telemetry?.egressMbps ?? 0} max={110_000} tone="danger" />
            <HudLinearBar label="CO2" value={telemetry?.cpuPercent ?? 0} max={100} tone="good" />
            <HudLinearBar label="FAN" value={telemetry?.ingressMbps ?? 0} max={110_000} tone="warn" />
          </div>
        </HudTile>
        ...
        <HudTile label="Level bank · thermal · power · air" className="holo-level-bank">
          <VerticalMeterBank
            ...
            meters={[
              ...
              { label: 'PWR', value: Math.min(100, (telemetry?.watts ?? 0) / 20), unit: '%' },
          ...
          <div className="holo-side-grid">
            ...
            <MultiRingGauge
              ...
              rings={[
                ...
                { label: 'PWR', value: Math.min(100, (telemetry?.watts ?? 0) / 20), color: 'good' },
