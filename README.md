...
| `make iso` fails with `client version 1.42 is too old. Minimum supported API version is 1.44` | BCI zypper `docker` CLI is too old for Docker 29+ host daemons | Rebuild iso-builder: `docker rmi harvester-nexus-iso-builder:1.0.0-nexus.1 && make iso-builder` (installs Docker 29 CLI from download.docker.com) |
