...
| `make iso` fails with `error waiting for container: unexpected EOF` on `COPY files/ /` | BuildKit + docker.sock from inside iso-builder, or Docker daemon OOM during harvester-os build | Rebuild on current branch (`DOCKER_BUILDKIT=0`, cockpit shipped as `dist.tar.zst`, full `build/` mounted). Ensure ≥25 GB free under `/var/lib/docker` and ≥8 GB RAM/swap |
