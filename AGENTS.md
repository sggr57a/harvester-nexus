...
| Unit tests | `npm run test` (Vitest, 51 tests) |
...
- Eight themes are available (Route Grid, Emerald Console, Arctic Hologram, Noir Radar, Tactical NVG, Ice Spectrum, Cyber Wireframe, Arctic Command) — switchable via the sidebar dropdown; persisted in `localStorage` as `nexus.theme`.
